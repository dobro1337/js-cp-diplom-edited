...
for(let i =0;i<wrapper.childElementCount; i++ ){ //перебираем каждое место и назначаем на него функцию нажатия
    let chairs = wrapperChildren[i].children;
    for(let j =0; j<chairs.length;j++){
        chairs[j].onclick = function () {// нажатие на место
...
var buyingInfoHint = document.getElementsByClassName('buying__info-hint')[0];// увеличение блока мест для маленьких экрановв
var lastTouchEnd = 0;
let scale = "scale(1.0)";
let rem = "3rem";
let marginTop = "0";
